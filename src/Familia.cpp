...
{
    salida << "Familia " << familia.obtener_id() << " con " << familia.obtener_miembros() << " miembros" << endl;
    salida << "Dias de preferencia [";

    for(unsigned int indice = 0; indice < familia.obtener_cantidad_dias_elegidos(); indice++) {
        salida << familia.obtener_dia_preferido_en(indice) << ",";
    }
    salida << "]" << endl;
